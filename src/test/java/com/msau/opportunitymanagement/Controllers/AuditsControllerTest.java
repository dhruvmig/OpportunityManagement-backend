...
public class AuditsControllerTest {
    ...
    @Test
    public void shouldGetAllLogs() throws Exception {
        Mockito.when(logDaoImp.getLogs()).thenReturn(new ArrayList<Logs>());
        mockMvc.perform(get("/audits/all")).andExpect(status().isOk());
    }
    ...
    public void shouldAddLogs() throws Exception{
        ...
        mockMvc.perform(post("/addAudits")
    ...
    @Test
    public void shouldGetAuditsWithId() throws Exception {
        Mockito.when(logDaoImp.getSpecificLogs(0)).thenReturn(new ArrayList<Logs>());
        mockMvc.perform(get("/getAudit/0")).andExpect(status().isOk());
    }
